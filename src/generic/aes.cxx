...
using uint8x4_t = std::array<uint8_t, 4>;
...
{
  unsigned i, j, k;
  uint8x4_t tempa; // Used for the column/row operations
...
inline static void XorWithIv(aes_block_t& buf, const aes_block_t& Iv) {
    for (auto i = 0U; i < buf.size(); ++i) {// The block in AES is always 128bit no matter the key size
        buf[i] ^= Iv[i];
    }
}
...
void AES_init_ctx(AES_ctx &ctx, const aes_key_t &key)
{
  KeyExpansion(ctx.RoundKey, key);
}
void AES_init_ctx_iv(AES_ctx &ctx, const aes_key_t &key, const aes_block_t &iv)
{
  KeyExpansion(ctx.RoundKey, key);
  std::copy_n(iv.begin(), iv.size(), ctx.Iv.begin());
}
void AES_ctx_set_iv(AES_ctx &ctx, const aes_block_t &iv)
{
  std::copy_n(iv.begin(), iv.size(), ctx.Iv.begin());
}

/* Symmetrical operation: same function for encrypting as for decrypting. Note any IV/nonce should never be reused with the same key */
inline static void AES_CTR_xcrypt_buffer(AES_ctx &ctx, aes_block_t* buf, size_t length)
{
    union {
        aes_block_t block;
        state_t state;
    ...
    auto limit = buf + length;
    for (auto current = buf; current < limit; ++current) {
...
{
    AES_ctx ctx{};
    AES_init_ctx_iv(ctx, sk, counter);
    AES_CTR_xcrypt_buffer(ctx, out.data(), out.size());
}

void aesctr256_zeroiv_software (std::vector<aes_block_t> &out, const aes_key_t &sk) {
    aes_block_t counter{};
    aesctr256_software(out, sk, counter);
}
